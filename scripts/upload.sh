...
curl -X POST -H "collection: collection1" -H "Content-MD5: $data1_md5" -H "Authorization: Signature keyId=\"rsa-key-1\",algorithm=\"hs2019\",headers=\"(request-target) (created) host digest content-length\",signature=\"Base64(RSA-SHA512(signing string))" --data-binary @data1.bin http://localhost:8081/collections &
